 const fs = require("fs");
 const canvas = require(`${__dirname}/../alice/wrapper/leiamnashF.js`).makeShip;
...
async function ship(event, api, leiam, log, alice) {
try{
const leiamObj = []
const leiamFile = __dirname + "/cache/ship_" + event.senderID + ".png";
...
await canvas({
  ...
  api.chat({body: `congratulations ${leiamUser} you have been shipped to ${leiamName} with ${leiamRate} chance becoming your future lover`, mentions: leiamObj, attachment: fs.createReadStream(leiamFile)}, event.threadID, (err) => {
