...
async function sim(event, api, leiam, log, alice) {
try{
const aliceFile = __dirname + "/cache/leiamnash_" + event.senderID + ".png";
const leiamChat = leiam.join(" ");
  if (!leiamChat) {
  await alice(event.senderID);
    api.chat({body: `context cannot be left blank\n\nhow to use?\n${global.alice.prefix}sim ⟨ context ⟩\n\nexample:\n${global.alice.prefix}sim hi`, attachment: fs.createReadStream(aliceFile)}, event.threadID, (err) => {
  fs.unlinkSync(aliceFile);
    if (err) return api.chat(`context cannot be left blank\n\nhow to use?\n${global.alice.prefix}sim ⟨ context ⟩\n\nexample:\n${global.alice.prefix}sim hi`, event.threadID, event.messageID);
    }, event.messageID);
         return;
}
