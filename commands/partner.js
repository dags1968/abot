 const fs = require("fs");
 const canvas = require(`${__dirname}/../alice/wrapper/leiamnashF.js`).makePartner;
...
async function partner(event, api, leiam, log, alice) {
try{
const leiamObj = []
const leiamFile = __dirname + "/cache/partner_" + event.senderID + ".png";
...
await canvas({
  ...
  api.chat({body: `congratulations ${leiamUser} you found ${leiamName} with ${leiamRate} chance becoming your future partner`, mentions: leiamObj, attachment: fs.createReadStream(leiamFile)}, event.threadID, (err) => {
