...
async function spotify(event, api, leiam, log, alice) {
try{
const leiamFile = __dirname + "/cache/spotify_" + event.senderID + ".mp3";
const aliceFile = __dirname + "/cache/leiamnash_" + event.senderID + ".png";
const leiamChat = leiam.join(" ");
  if (!leiamChat) {
  await alice(event.senderID);
    api.chat({body: `search cannot be left blank\n\nhow to use?\n${global.alice.prefix}spotify ⟨ search ⟩\n\nexample:\n${global.alice.prefix}spotify forever lewis capaldi`, attachment: fs.createReadStream(aliceFile)}, event.threadID, (err) => {
  fs.unlinkSync(aliceFile);
    if (err) return api.chat(`search cannot be left blank\n\nhow to use?\n${global.alice.prefix}spotify ⟨ search ⟩\n\nexample:\n${global.alice.prefix}spotify forever lewis capaldi`, event.threadID, event.messageID);
    }, event.messageID);
         return;
 }
  const leiamRes = (await axios.get(`${global.alice.api}/spotify?search=${leiamChat}`)).data;
