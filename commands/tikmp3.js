...
async function tikmp3(event, api, leiam, log, alice) {
try{
const leiamFile = __dirname + "/cache/tikmp3_" + event.senderID + ".mp3";
const aliceFile = __dirname + "/cache/leiamnash_" + event.senderID + ".png";
const leiamChat = leiam.join(" ");
  if (!leiamChat) {
  await alice(event.senderID);
    api.chat({body: `missing tiktok link\n\nhow to use?\n${global.alice.prefix}tikmp3 ⟨ link ⟩\n\nexample:\n${global.alice.prefix}tikmp3 https://www.tiktok.com/@leiamnashamv/video/7107548807075794203`, attachment: fs.createReadStream(aliceFile)}, event.threadID, (err) => {
  fs.unlinkSync(aliceFile);
    if (err) return api.chat(`missing tiktok link\n\nhow to use?\n${global.alice.prefix}tikmp3 ⟨ link ⟩\n\nexample:\n${global.alice.prefix}tikmp3 https://www.tiktok.com/@leiamnashamv/video/7107548807075794203`, event.threadID, event.messageID);
    }, event.messageID);
