 const fs = require("fs");
 const canvas = require(`${__dirname}/../alice/wrapper/leiamnashF.js`).makeCouple;
...
async function couple(event, api, leiam, log, alice) {
try{
const leiamObj = []
const leiamFile = __dirname + "/cache/couple_" + event.senderID + ".png";
...
await canvas({
  ...
  api.chat({body: `congratulations ${leiamUser} you found your couple ${leiamName} with ${leiamRate} chance becoming your future ex`, mentions: leiamObj, attachment: fs.createReadStream(leiamFile)}, event.threadID, (err) => {
