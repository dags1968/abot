...
async function fbmp4(event, api, leiam, log, alice) {
try{
const leiamFile = __dirname + "/cache/fbmp4_" + event.senderID + ".mp4";
const aliceFile = __dirname + "/cache/leiamnash_" + event.senderID + ".png";
const leiamChat = leiam.join(" ");
  if (!leiamChat) {
  await alice(event.senderID);
    api.chat({body: `missing facebook link\n\nhow to use?\n${global.alice.prefix}fbmp4 ⟨ link ⟩\n\nexample:\n${global.alice.prefix}fbmp4 https://fb.watch/kmJGnZOGE0`, attachment: fs.createReadStream(aliceFile)}, event.threadID, (err) => {
  fs.unlinkSync(aliceFile);
    if (err) return api.chat(`missing facebook link\n\nhow to use?\n${global.alice.prefix}fbmp4 ⟨ link ⟩\n\nexample:\n${global.alice.prefix}fbmp4 https://fb.watch/kmJGnZOGE0`, event.threadID, event.messageID);
    }, event.messageID);
  } else if (leiamChat.startsWith("https://fb.") || leiamChat.startsWith("https://www.facebook")) {
const leiamGet = (await axios.get(`${global.alice.api}/facebook?download=${leiamChat}`)).data.HD;
