 const fs = require("fs");
 const canvas = require(`${__dirname}/../alice/wrapper/leiamnashB.js`).makeClown;
...
async function clown(event, api, leiam, log) {
try{
 const alice = leiam.join(" ");
 const leiamnash = Object.keys(event.mentions);
 const file = __dirname + "/cache/clown_" + event.senderID + ".png";
 const leiamTxt = "you're not a clown you're the entire circus";
