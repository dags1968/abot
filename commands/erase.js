 const fs = require("fs");
 const canvas = require(`${__dirname}/../alice/wrapper/leiamnashB.js`).makeErase;
...
async function erase(event, api, leiam, log) {
try{
 const alice = leiam.join(" ");
 const leiamnash = Object.keys(event.mentions);
 const file = __dirname + "/cache/erase_" + event.senderID + ".png";
 const leiamTxt = "it's time to erase some garbage stuff";
