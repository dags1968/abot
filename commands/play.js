...
async function play(event, api, leiam, log, alice) {
try{
const leiamFile = __dirname + "/cache/play_" + event.senderID + ".mp4";
const aliceFile = __dirname + "/cache/leiamnash_" + event.senderID + ".png";
const leiamChat = leiam.join(" ");
  if (!leiamChat) {
  await alice(event.senderID);
    api.chat({body: `search cannot be left blank\n\nhow to use?\n${global.alice.prefix}play ⟨ search ⟩\n\nexample:\n${global.alice.prefix}play forever lewis capaldi`, attachment: fs.createReadStream(aliceFile)}, event.threadID, (err) => {
  fs.unlinkSync(aliceFile);
    if (err) return api.chat(`search cannot be left blank\n\nhow to use?\n${global.alice.prefix}play ⟨ search ⟩\n\nexample:\n${global.alice.prefix}play forever lewis capaldi`, event.threadID, event.messageID);
    }, event.messageID);
         return;
 }
...
fs.writeFileSync(leiamFile, Buffer.from(leiamGet, 'utf-8'));
  api.chat({body: `${leiamRes.title}`, attachment: fs.createReadStream(leiamFile)}, event.threadID, (err) => {
