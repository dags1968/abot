 const fs = require("fs");
 const canvas = require(`${__dirname}/../alice/wrapper/leiamnashB.js`).makeWanted;
...
async function wanted(event, api, leiam, log) {
try{
 const alice = leiam.join(" ");
 const leiamnash = Object.keys(event.mentions);
 const file = __dirname + "/cache/wanted_" + event.senderID + ".png";
 const leiamTxt = "you stole my heart and now im dead";
