 const fs = require("fs");
 const canvas = require(`${__dirname}/../alice/wrapper/leiamnashB.js`).makeAffect;
...
async function affect(event, api, leiam, log) {
try{
 const alice = leiam.join(" ");
 const leiamnash = Object.keys(event.mentions);
 const file = __dirname + "/cache/affect_" + event.senderID + ".png";
  const leiamTxt = "yes it's doesn't affect at all";
