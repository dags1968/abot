 const fs = require("fs");
 const canvas = require(`${__dirname}/../alice/wrapper/leiamnashB.js`).makeRip;
...
async function rip(event, api, leiam, log) {
try{
 const alice = leiam.join(" ");
 const leiamnash = Object.keys(event.mentions);
 const file = __dirname + "/cache/rip_" + event.senderID + ".png";
 const leiamTxt = "condolence i wish your soul will go to hell";
