 const fs = require("fs");
 const canvas = require(`${__dirname}/../alice/wrapper/leiamnashB.js`).makeGangbang;
...
async function gangbang(event, api, leiam, log, alice) {
try{
...
api.chat({body: "sorry but this command is only available for premium users only and you don't have an premium access to use this", attachment: fs.createReadStream(aliceFile)}, event.threadID, (err) => {
 ...
 const file = __dirname + "/cache/gangbang_" + event.senderID + ".png";
