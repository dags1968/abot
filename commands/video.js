...
async function video(event, api, leiam, log, alice) {
try{
...
const leiamChat = leiam.join(" ");
  ...
  await alice(event.senderID);
    api.chat({body: `search cannot be left blank\n\nhow to use?\n${global.alice.prefix}video ⟨ search ⟩\n\nexample:\n${global.alice.prefix}video forever lewis capaldi`, attachment: fs.createReadStream(aliceFile)}, event.threadID, (err) => {
  fs.unlinkSync(aliceFile);
    if (err) return api.chat(`search cannot be left blank\n\nhow to use?\n${global.alice.prefix}video ⟨ search ⟩\n\nexample:\n${global.alice.prefix}video forever lewis capaldi`, event.threadID, event.messageID);
    }, event.messageID);
         return;
 }
