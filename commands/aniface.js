 const fs = require("fs");
 const canvas = require(`${__dirname}/../alice/wrapper/leiamnashB.js`).makeAniface;
...
async function aniface(event, api, leiam, log) {
try{
 const alice = leiam.join(" ");
 const leiamnash = Object.keys(event.mentions);
 const file = __dirname + "/cache/aniface_" + event.senderID + ".png";
if(event.type == "message_reply") {
 ...
 const leiamGet = (await api.getUserInfo(event.messageReply.senderID))[event.messageReply.senderID].name;
    api.chat({body: leiamGet, mentions: [{tag: leiamGet, id: event.messageReply.senderID}], attachment: fs.createReadStream(file)}, event.threadID, (err) => {
   fs.unlinkSync(file);
  ...
  const leiamGet = (await api.getUserInfo(event.senderID))[event.senderID].name;
    api.chat({body: leiamGet, mentions: [{tag: leiamGet, id: event.senderID}], attachment: fs.createReadStream(file)}, event.threadID, (err) => {
   fs.unlinkSync(file);
    if (err) return api.chat(`Error: {\nstatus: 3792\nsummary: {\n'leiamnash server is offline',\n'this is temporary issue please request again'\n'undefined leiamnash server'\n},\nalicezetion: this error happens if your account get muted by facebook\n}`, event.threadID, event.messageID);
    }, event.messageID);
 } else if (leiamnash) {
  ...
  const leiamGet = (await api.getUserInfo(leiamnash))[leiamnash].name;
    api.chat({body: leiamGet, mentions: [{tag: leiamGet, id: leiamnash[0]}], attachment: fs.createReadStream(file)}, event.threadID, (err) => {
   fs.unlinkSync(file);
    if (err) return api.chat(`Error: {\nstatus: 3792\nsummary: {\n'leiamnash server is offline',\n'this is temporary issue please request again'\n'undefined leiamnash server'\n},\nalicezetion: this error happens if your account get muted by facebook\n}`, event.threadID, event.messageID);
    }, event.messageID);
  }
